...
class Scene {
  constructor(_movementCallback) {
    ...
    // add controls:
    this.controls = new THREE.PlayerControls(this.camera, this.playerGroup, this.scene);
  ...
  addClient(_id) {
    // let characterMaterial = getCharacterMaterial(id for the character chosen);

    let _head = new THREE.Mesh(new THREE.PlaneGeometry(2, 2.5, 2), );
...
/*
FOR USE LATER
function getCharacterMaterial(_id) {
  let characterTexture = THREE.ImageUtils.loadTexture( "../img/texture.jpg" );

  let characterMaterial = new THREE.MeshBasicMaterial({
